...
public class CollectDemandOnProductTop extends BaseBean implements Action {
    /**
    ...
    public String execute(RequestInfo request) {
        ...
        //bm2默认值是-1
        //如果填报人所在的营销公司是三层结构，就会更新bm2的值
        String bm2 ="-1";
        bm2 = rs.getString("bm2");
        //writeLog("申请人部门2：" + Integer.parseInt(bm2));

        //如果填报人所在的营销公司是两层结构，即bm2为空，就把bm1的值赋给bm2，以便更新数据
        //如果是三层，即bm2buweikong，就不需要赋值
        if("".equals(bm2))
            bm2=bm1;
        writeLog("申请人部门2：" + bm2);
...
//                "END;";
            ...
            SQL+="IF NOT EXISTS (select * from uf_CollDmdOnProd where year='"+year+"' and month="+month+" and bm="+bm2+" and MatID="+wlbmid+") " +
                    "BEGIN " +
                        "INSERT into uf_CollDmdOnProd (Year,Month,fb,bm,MatID,Amount,formmodeid,modedatacreatedate)" +
                        "values('"+year+"','"+month+"','"+fb+"','"+bm2+"','"+wlbmid+"','"+amount+"','"+"127"+"',CAST(GETDATE() AS DATE)) " +
                    "END " +
                    "ELSE " +
                    "BEGIN " +
                        "UPDATE uf_CollDmdOnProd SET Amount=Amount+"+amount+
                        " WHERE year='"+year+"' and month="+month+" and bm="+bm2+" and MatID="+wlbmid+
                    " END ";
        ...
        {
            writeLog("销售计划填报流程正常-Top节点 requestid:"+requestId);
            //writeLog(SQL);
        }
        else
        {
            writeLog("销售计划填报流程出错-Top节点 requestid:"+requestId);
        }
