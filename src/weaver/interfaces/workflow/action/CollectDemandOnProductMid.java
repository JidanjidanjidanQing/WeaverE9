...
public class CollectDemandOnProductMid extends BaseBean implements Action {
    /**
...
//                "END;";
            ...
            SQL+="IF NOT EXISTS (select * from uf_CollDmdOnProd where year='"+year+"' and month="+month+" and bm="+bm1+" and MatID="+wlbmid+") " +
                    "BEGIN " +
                        "INSERT into uf_CollDmdOnProd (Year,Month,fb,bm,MatID,Amount,formmodeid,modedatacreatedate)" +
                        "values('"+year+"','"+month+"','"+fb+"','"+bm1+"','"+wlbmid+"','"+amount+"','"+"127"+"',CAST(GETDATE() AS DATE)) " +
                    "END " +
                    "ELSE " +
                    "BEGIN " +
                        "UPDATE uf_CollDmdOnProd SET Amount=Amount+"+amount+
                        " WHERE year='"+year+"' and month="+month+" and bm="+bm1+" and MatID="+wlbmid+
                    " END ";
        ...
        {
            writeLog("销售计划填报流程正常-Mid节点 requestid:"+requestId);
            //writeLog(SQL);
        }
        else
        {
            writeLog("销售计划填报流程出错-Mid节点 requestid:"+requestId);
        }
